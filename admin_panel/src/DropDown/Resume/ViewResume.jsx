...
export default function ViewResume() {
    ...
    let resumeDataView = () => {
        axios.post("https://dynmic-portfolio-my-website.onrender.com/api/backend/Resume/view")
            .then((res) => {
    ...
    const deleteResume = (id) => {
        if (window.confirm("Are you sure you want to delete this resume?")) {
            axios.delete(`https://dynmic-portfolio-my-website.onrender.com/api/backend/Resume/delete/${id}`)
                .then((res) => {
    ...
    const statusUpdate = (id, currentStatus) => {
        ...
        axios.post("https://dynmic-portfolio-my-website.onrender.com/api/backend/Resume/status-change", obj)
            .then((res) => {
                console.log(res.data)
                resumeDataView()
            })
    ...
    const filteredData = resumeData.filter(resume =>
        ...
        (resume.status ? "active" : "inactive").includes(searchTerm.toLowerCase())
    );
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b border-slate-800 bg-slate-950/50">
                                            ...
                                            <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Preview</th>
                                            <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">File Type</th>
                                    ...
                                    <tbody className="divide-y divide-slate-800">
                                        {filteredData.length > 0 ? (
                                            filteredData.map((resume, index) => (
                                                <tr key={resume._id} className="hover:bg-slate-800/30 transition-colors group/row">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="relative w-16 h-16 rounded-lg overflow-hidden border border-slate-700 bg-slate-950 flex items-center justify-center">
                                                            {resume.file_type === "raw" ? (
                                                                <FaFilePdf className="text-red-400 w-8 h-8" title="PDF File" />
                                                            ) : (
                                                                <img
                                                                    src={resume.image}
                                                                    alt="Resume"
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${resume.file_type === "raw" ? "bg-red-500/10 text-red-400" : "bg-indigo-500/10 text-indigo-400"
                                                            }`}>
                                                            {resume.file_type === "raw" ? <FaFilePdf size={10} /> : <FaImage size={10} />}
                                                            {resume.file_type === "raw" ? "PDF Document" : "Image Format"}
                                                        </span>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                        <button
                                                            onClick={() => statusUpdate(resume._id, resume.status)}
                                            ...
                                            <tr>
                                                <td colSpan="5" className="px-6 py-12 text-center text-slate-500 italic">
                                                    {searchTerm ? "No resumes matching your search." : "No resumes found. Start by uploading one!"}
                                                </td>
                                            </tr>
                                        )}
                                    </tbody>
